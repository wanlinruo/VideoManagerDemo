...
class ThumbExoPlayerView(context: Context, attrs: AttributeSet?) : PlayerView(context, attrs) {
    ...
    fun setDataSource(
        ...
        mediaPath = source
        exoPlayer = initPlayer(context, mediaPath, this, listener)
        exoPlayer?.volume = 0f
...
//        exoPlayer?.playWhenReady = false
        ...
        Thread {
            val duration = getVideoDuration(context, mediaPath)
    ...
    private fun startPlayAndCapture() {
        ...
        val timeMs = thumbnailMillSecList[0]
...
//            Log.d(TAG, "startPlayAndCapture()  bitmap:$bitmap")
            bitmap?.run {
                context.externalCacheDir?.let {
                    val fileName = it.absolutePath + "thumbnail_" + bitmapIndex
